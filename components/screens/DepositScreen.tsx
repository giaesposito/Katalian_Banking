...
const DepositScreen: React.FC<DepositScreenProps> = ({ user, onNavigate, onDeposit }) => {
    ...
    const handleDeposit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        // Simulate real API provisioning time
        setTimeout(async () => {
                    ...
                    <div className="space-y-8 animate-in fade-in duration-500">
                        ...
                        <div className="bg-emerald-500/5 border border-emerald-500/10 p-8 rounded-3xl space-y-4">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-sm font-bold text-white">CHASE BANK NA</span>
                            </div>
                            <div className="flex justify-between items-center">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-sm font-bold text-emerald-500 italic">Immediate</span>
                            </div>
                        </div>
                        <p className="text-[10px] text-slate-500 text-center uppercase font-black tracking-widest leading-relaxed">
                            By proceeding, you authorize Katalian Bank to initiate an ACH transfer from the linked source.
                        </p>
                    </div>
                );
            case 3:
                return (
                    <div className="space-y-8 animate-in fade-in duration-500">
                        <div className="space-y-2 text-center">
                            ...
                            <p className="text-slate-500 text-sm font-medium">Please review the details below before authorizing the deposit.</p>
                        </div>
                        <div className="bg-slate-950/50 p-8 rounded-3xl border border-white/5 space-y-6">
                            <div className="text-center pb-6 border-b border-white/5">
                                ...
                                <p className="text-5xl font-black text-white tracking-tighter tabular-nums">${parseFloat(amount).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                            </div>
                            <div className="space-y-4">
                                <div className="flex justify-between">
                                    <span className="text-xs font-bold text-slate-500">To Account</span>
                                    <span className="text-xs font-bold text-white">
                                        {user.accounts.find(a => a.id === toAccountId)?.type}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-xs font-bold text-slate-500">Processing Fee</span>
                                    <span className="text-xs font-bold text-emerald-500">$0.00 (Waived)</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="text-center space-y-10 py-16 animate-in fade-in zoom-in-95 duration-700">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-slate-400 max-w-sm mx-auto font-medium">
                                We've successfully pulled ${parseFloat(amount).toLocaleString()} into your {user.accounts.find(a => a.id === toAccountId)?.type} account.
                            </p>
                        </div>
                        <Button onClick={() => onNavigate({name:'dashboard'})} variant="primary" className="px-12 py-4 !rounded-full">Back to Dashboard</Button>
                    </div>
                );
            default: return null;
        }
                ...
                <div className="flex justify-between items-center mb-16">
                    <div className="space-y-1">
                        <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic">
                            Deposit <span className="text-slate-500 font-normal">Facility</span>
                        </h2>
                ...
                {loading ? (
                    <div className="py-24 flex flex-col items-center justify-center space-y-8 animate-in fade-in duration-500">
                        ...
                        <div className="text-center space-y-2">
                            <h4 className="text-xl font-black text-white uppercase tracking-widest">Establishing Secure Link</h4>
                            <p className="text-slate-500 text-sm font-medium">Validating liquidity source and updating ledgers...</p>
                        </div>
                    </div>
                ) : (
